...
-(void)updatePhotoOfUserHavingZUID:(NSString*)zuid
    ...
    [self getTokenForZUID:zuid WithSuccess:^(NSString *token) {
        NSString *accountsURL = [self getAccountsURLFromKeychainForZUID:[self getCurrentUserZUIDFromKeychain]];
        NSString *profileBaseURL = [accountsURL stringByReplacingOccurrencesOfString:@"accounts" withString:@"profile"];
        NSString *urlString = [NSString stringWithFormat:@"%@/api/v1/user/self/photo",profileBaseURL];
...
-(void)presentAccountChooserWithSuccess:(requestSuccessBlock)success andFailure:(requestFailureBlock)failure havingSwitchSuccess:(ZSSOKitManageAccountsSuccessHandler)switchSuccess{
    ...
    accountListTableViewController.usersCount = count;
