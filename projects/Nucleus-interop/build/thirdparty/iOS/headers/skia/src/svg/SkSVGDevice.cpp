...
static SkScalar svg_opacity(SkColor color) {
    SkScalar opacity = SkIntToScalar(SkColorGetA(color)) / SK_AlphaOPAQUE;
...
auto svg_toString = [](const char* type) { return std::string(type); };

const char* svg_bendMode(SkBlendMode mode) {
    const char* blendMode = SkBlendMode_Name(mode);
    if (blendMode == svg_toString("ColorDodge")) blendMode = "Color-Dodge";
    if (blendMode == svg_toString("ColorBurn")) blendMode = "Color-Burn";
    if (blendMode == svg_toString("HardLight")) blendMode = "Hard-Light";
    if (blendMode == svg_toString("SoftLight")) blendMode = "Soft-Light";
    return blendMode;
}
...
public:
    ...
    SkString addShadowFilter() {
        return SkStringPrintf("shadow_%d", fShadowFilterCount++); 
    }

    SkString addMaskFilter() { 
        return SkStringPrintf("mask_%d", fMaskFilterCount++); 
    }
...
private:
    ...
    uint32_t fShadowFilterCount;
    uint32_t fMaskFilterCount;
};
...
private:
    ...
    void addImageShaderResources(const SkShader* shader, const SkPaint& paint,
                                 Resources* resources);
    void addColorShader(SkShader* shader, const SkPaint& paint, Resources* resources);
    ...
    SkString addSweepGradientDef(const SkShaderBase::GradientInfo& info,
                                 const SkShader* shader,
                                 const SkMatrix& localMatrix);
    SkShaderBase* getBlenderShader(SkShader* shader);

    void addStandAloneShaders(SkShader* shader, const SkPaint& paint, Resources* resources);

    SkXMLWriter* fWriter;
    ResourceBucket* fResourceBucket;
};

SkShaderBase* SkSVGDevice::AutoElement::getBlenderShader(SkShader* shader) {
    SkShaderBase* baseShader = as_SB(shader);
    SkColorFilterShader* colorFilterShader = static_cast<SkColorFilterShader*>(baseShader);
    return static_cast<SkShaderBase*>(colorFilterShader->getShader());
}
...
void SkSVGDevice::AutoElement::addPaint(const SkPaint& paint, const Resources& resources) {
    ...
    if (style == SkPaint::kFill_Style || style == SkPaint::kStrokeAndFill_Style) {
        ...
        } else if (!svg_color(paint.getColor()).equals(kDefaultFill)) {
            this->addAttribute("fill", svg_color(paint.getColor()));
        }
        if (SK_AlphaOPAQUE != SkColorGetA(paint.getColor())) {
            this->addAttribute("fill-opacity", svg_opacity(paint.getColor()));
        } else if (svg_opacity(SK_AlphaOPAQUE) != resources.opacity) {
            this->addAttribute("fill-opacity", resources.opacity);
        }
    } else {
    ...
    if (paint.getBlendMode_or(SkBlendMode::kSrcOver) != SkBlendMode::kSrcOver) {
        SkString blend;
        blend.appendf("mix-blend-mode: %s",
                      SkBlendMode_Name(paint.getBlendMode_or(SkBlendMode::kSrcOver)));
        this->addAttribute("style", blend);
    }
    ...
    if (style == SkPaint::kStroke_Style || style == SkPaint::kStrokeAndFill_Style) {
        ...
        if (SK_AlphaOPAQUE != SkColorGetA(paint.getColor())) {
            ...
            this->addAttribute("stroke-opacity", resources.opacity);
        }
    } else {
...
void SkSVGDevice::AutoElement::addGradientShaderResources(const SkShader* shader,
    ...
    if (gradient_type == SkShaderBase::GradientType::kLinear) {
        resources->fPaintServer.printf("url(#%s)",
                                       addLinearGradientDef(grInfo, shader, localMatrix).c_str());
    } else if (gradient_type == SkShaderBase::GradientType::kRadial) {
        resources->fPaintServer.printf("url(#%s)",
                                       addRadialGradientDef(grInfo, shader, localMatrix).c_str());
    } else if (gradient_type == SkShaderBase::GradientType::kSweep) {
        resources->fPaintServer.printf("url(#%s)",
                                       addSweepGradientDef(grInfo, shader, localMatrix).c_str());
    }
}
...
void SkSVGDevice::AutoElement::addColorShader(SkShader* shader,
                                              const SkPaint& paint,
                                              Resources* resources) {
    SkColor color = as_SB(shader)->getShaderColor();
    SkString colorStr(svg_color(color));
    SkScalar alphaValue = svg_opacity(color);
    resources->fPaintServer.printf("%s", colorStr.c_str());
    if (alphaValue != 0) {
        resources->opacity = alphaValue;
    }
}

void SkSVGDevice::AutoElement::addImageShaderResources(const SkShader* shader,
                                                       const SkPaint& paint,
                                                       Resources* resources) {
    SkMatrix outMatrix;
    ...
    SkIRect imageRect = image->bounds();
    for (int i = 0; i < 2; i++) {
    ...
    SkString patternID = fResourceBucket->addPattern();
    {
        ...
        pattern.addAttribute("x", imageRect.fLeft);
        pattern.addAttribute("y", imageRect.fTop);

        {
            ...
            imageTag.addAttribute("width", imageRect.width());
            imageTag.addAttribute("height", imageRect.height());
...
void SkSVGDevice::AutoElement::addStandAloneShaders(SkShader* shader,
                                                    const SkPaint& paint,
                                                    Resources* resources) {
    if (shader->isAImage()) {
        this->addImageShaderResources(shader, paint, resources);
    } else if (shader->isAColor()) {
        this->addColorShader(shader, paint, resources);
    } else if (as_SB(shader)->asGradient(nullptr) != SkShaderBase::GradientType::kNone) {
...
void SkSVGDevice::AutoElement::addShaderResources(const SkPaint& paint, Resources* resources) {
    ...
    if (shaderType == SkShaderBase::ShaderType::kColorFilter) {
        SkColorFilterShader* colorFilterShader = static_cast<SkColorFilterShader*>(shader);
        if (as_SB(colorFilterShader->getShader())->type() == SkShaderBase::ShaderType::kBlend) {
            SkShaderBase* blendShader = getBlenderShader(shader);
            SkShader* source = blendShader->getSourceShader();
            SkShader* dest = blendShader->getDestinationShader();
            if (source && dest) {
                addStandAloneShaders(dest, paint, resources);
                addStandAloneShaders(source, paint, resources);
            }
        } else {
            addStandAloneShaders(colorFilterShader->getShader(), paint, resources);
        }
    } else {
        addStandAloneShaders(shader, paint, resources);
    }
}
...
SkString SkSVGDevice::AutoElement::addRadialGradientDef(const SkShaderBase::GradientInfo& info,
...

    {
        ...
        gradient.addAttribute("id", id);
        gradient.addAttribute("gradientUnits", "userSpaceOnUse");
        gradient.addAttribute("r", SkStringPrintf("%f", info.fRadius[0]));
...
SkString SkSVGDevice::AutoElement::addSweepGradientDef(const SkShaderBase::GradientInfo& info,
                                                       const SkShader* shader,
                                                       const SkMatrix& localMatrix) {
    SkASSERT(fResourceBucket);
    SkString id = fResourceBucket->addGradient();

    {
        ...
        if (!localMatrix.isIdentity()) {
            gradient.addAttribute("gradientTransform", svg_transform(localMatrix));
        }
        ...
        for (int i = 0; i < info.fColorCount; ++i) {
            ...
            {
                AutoElement stop("stop", fWriter);
                float offset = static_cast<float>(i) / (info.fColorCount - 1);
                stop.addAttribute("offset", offset);
                stop.addAttribute("stop-color", colorStr.c_str());
...
void SkSVGDevice::syncClipStack(const SkClipStack& cs) {
    ...
    if (currentLayerFilter->size() > 0 && !currentLayerFilter->isLayerFilterAdded()) {
        SkPaint* paint = currentLayerFilter->getPaint();
        currentLayerFilter->getElement()->fWriter->startElement("g");
        if (!currentLayerFilter->returnId().empty()) {
            if (currentLayerFilter->returnType() == "shadow") {
                currentLayerFilter->getElement()->fWriter->addAttribute(
                        "filter", currentLayerFilter->returnId().c_str());
            } else {
                currentLayerFilter->getElement()->fWriter->addAttribute(
                        "clip-path", currentLayerFilter->returnId().c_str());
            }
        }
        if (paint->getAlphaf() != 1 && paint->getAlphaf() != 0) {
            currentLayerFilter->getElement()->fWriter->addScalarAttribute("opacity",
                                                                          paint->getAlphaf());
        }
        currentLayerFilter->addlayerFilter();
    }
}
...
// sk_sp<SkDevice> SkSVGDevice::createDevice(const CreateInfo& cinfo, const SkPaint* layerPaint) {
// }

void SkSVGDevice::onSaveLayer(SkPaint* paint) {
    auto generateImageFilter = [this](SkPaint* paint) {
        
        auto generateShadowFilter =
                [this](std::vector<std::shared_ptr<SkImageFilter::shadowAttributes>> shadows) {
                    SkString feBlendIn = SkStringPrintf("SourceGraphic");
                    SkString shadowFilterID = fResourceBucket->addShadowFilter();
                    {
                        AutoElement filterElement("filter", fWriter);
                        filterElement.addAttribute("id", shadowFilterID);
                        filterElement.addAttribute("x", "-1");
                        filterElement.addAttribute("y", "-1");
                        filterElement.addAttribute("width", shadows[0]->width);
                        filterElement.addAttribute("height", shadows[0]->height);
                        filterElement.addAttribute("color-interpolation-filters", "sRGB");

                        for (std::size_t i = 0; i < shadows.size(); i++) {
                            SkString currentShadowId =
                                    SkStringPrintf("shadow%d", int(shadows.size() - i));
                            if (strcmp(shadows[i]->type, "outer") == 0) {
                                {
                                    AutoElement blurElement("feGaussianBlur", fWriter);
                                    blurElement.addAttribute("in", "SourceGraphic");
                                    blurElement.addAttribute("stdDeviation", shadows[i]->sigma);
                                }
                                {
                                    AutoElement offsetElement("feOffset", fWriter);
                                    offsetElement.addAttribute("dy", shadows[i]->dx);
                                    offsetElement.addAttribute("dx", shadows[i]->dy);
                                    offsetElement.addAttribute("result", "offOut");
                                }
                                {
                                    AutoElement floodElement("feFlood", fWriter);
                                    floodElement.addAttribute("flood-color",
                                                              svg_color(shadows[i]->color));
                                    floodElement.addAttribute("flood-opacity",
                                                              svg_opacity(shadows[i]->color));
                                }
                                {
                                    AutoElement compositeElement("feComposite", fWriter);
                                    compositeElement.addAttribute("in2", "offOut");
                                    compositeElement.addAttribute("operator", "in");
                                    compositeElement.addAttribute("result",
                                                                  SkStringPrintf("shadow"));
                                }
                                {
                                    AutoElement offsetElement("feBlend", fWriter);
                                    offsetElement.addAttribute("in2", "shadow");
                                    offsetElement.addAttribute("in", feBlendIn);
                                    offsetElement.addAttribute("mode", "normal");
                                    offsetElement.addAttribute("result", currentShadowId);
                                }
                            } else {
                                {
                                    AutoElement offsetElement("feOffset", fWriter);
                                    offsetElement.addAttribute("dy", shadows[i]->dy);
                                    offsetElement.addAttribute("dx", shadows[i]->dx);
                                }
                                {
                                    AutoElement blurElement("feGaussianBlur", fWriter);
                                    blurElement.addAttribute("stdDeviation", shadows[i]->sigma);
                                    blurElement.addAttribute("result", "offset-blur");
                                }
                                {
                                    AutoElement compositeElement("feComposite", fWriter);
                                    compositeElement.addAttribute("in", "SourceGraphic");
                                    compositeElement.addAttribute("in2", "offset-blur");
                                    compositeElement.addAttribute("operator", "out");
                                    compositeElement.addAttribute("result", "inverse");
                                }
                                {
                                    AutoElement floodElement("feFlood", fWriter);
                                    floodElement.addAttribute("flood-color",
                                    ...
                                    floodElement.addAttribute("flood-opacity",
                                                              svg_opacity(shadows[i]->color));
                                    floodElement.addAttribute("result", "color");
                                }
                                {
                                    AutoElement compositeElement("feComposite", fWriter);
                                    compositeElement.addAttribute("in", "color");
                                    compositeElement.addAttribute("in2", "inverse");
                                    compositeElement.addAttribute("operator", "in");
                                    compositeElement.addAttribute("result", "shadow");
                                }
                                {
                                    AutoElement compositeElement("feComposite", fWriter);
                                    compositeElement.addAttribute("in", "shadow");
                                    compositeElement.addAttribute("in2", feBlendIn);
                                    compositeElement.addAttribute("operator", "over");
                                    compositeElement.addAttribute("result", currentShadowId);
                                }
                            }
                            feBlendIn = currentShadowId;
                        }
                    }
                    return shadowFilterID;
                };

        auto generateBlurFilter = [this](std::shared_ptr<SkImageFilter::shadowAttributes> blur) {
            SkString feBlendIn = SkStringPrintf("SourceGraphic");
            SkString shadowFilterID = fResourceBucket->addShadowFilter();
            {
                AutoElement filterElement("filter", fWriter);
                ...
                filterElement.addAttribute("filterUnits", "userSpaceOnUse");
                {
                    AutoElement blurElement("feGaussianBlur", fWriter);
                    blurElement.addAttribute("in", "SourceGraphic");
                    blurElement.addAttribute("stdDeviation", blur->sigma);
                }
            }
            return shadowFilterID;
        };

        SkImageFilter* img = paint->getImageFilter();
        SkString url;
        // for blur
        if (std::string(img->getTypeName()) == "SkBlurImageFilter") {
            auto res = SkImageFilter::extractBlurAttributes(img);
            url.printf("url(#%s)", generateBlurFilter(res).c_str());
            return url;
        } else if (std::string(img->getTypeName()) == "SkComposeImageFilter") {  // for blur + shadows
            if (std::string(img->getInput(0)->getTypeName()) == "SkBlurImageFilter") {
                auto res = SkImageFilter::extractBlurAttributes(const_cast<SkImageFilter*>(img->getInput(0)));
                url.printf("url(#%s)", generateBlurFilter(res).c_str());
            }
            if (std::string(img->getInput(1)->getTypeName()) == "SkBlendImageFilter") {
                std::vector<std::shared_ptr<SkImageFilter::shadowAttributes>> shadows;
                SkImageFilter* shadowImg = const_cast<SkImageFilter*>(img->getInput(1));

                while (shadowImg) {
                    if (SkImageFilter::isDropShadowFilter(const_cast<SkImageFilter*>(shadowImg->getInput(0)))) {
                        shadows.push_back(SkImageFilter::extractOuterShadowAttributes(shadowImg));
                    } else if (SkImageFilter::isInnerShadowFilter(const_cast<SkImageFilter*>(shadowImg->getInput(0)))) {
                        shadows.push_back(SkImageFilter::extractInnerShadowAttributes(const_cast<SkImageFilter*>(shadowImg)));
                    }
                    shadowImg = const_cast<SkImageFilter*>(shadowImg->getInput(1));
                }
                url.printf("url(#%s)", generateShadowFilter(shadows).c_str());
            }
            return url;
        } else if (img->getInput(0)) { // For standalone shadows
            
            std::vector<std::shared_ptr<SkImageFilter::shadowAttributes>> shadows;
            while (img) {
                if (SkImageFilter::isDropShadowFilter(const_cast<SkImageFilter*>(img->getInput(0)))) {
                    shadows.push_back(SkImageFilter::extractOuterShadowAttributes(img));
                } else if (SkImageFilter::isInnerShadowFilter(const_cast<SkImageFilter*>(img->getInput(0)))) {
                    shadows.push_back(SkImageFilter::extractInnerShadowAttributes(img));
                }
                img = const_cast<SkImageFilter*>(img->getInput(1));
            }
            url.printf("url(#%s)", generateShadowFilter(shadows).c_str());
            return url;
        }
    ...
    if (currentLayerFilter) {
        if (((paint->getAlphaf() != 1 && paint->getAlphaf() != 0) && paint->getImageFilter()) ||
            paint->getImageFilter()) {
            auto filterId = generateImageFilter(paint);
            if (!filterId.isEmpty()) {
                if (static_cast<std::size_t>(filterId.find("shadow")) != std::string::npos) {
                    currentLayerFilter->push(paint, this, filterId.c_str(), "shadow");
                } else {
                    currentLayerFilter->push(paint, this, filterId.c_str(), "mask");
                }
            }
        } else if (paint->getAlphaf() != 1 && paint->getAlphaf() != 0) {
            currentLayerFilter->push(paint, this);
        }
    }
}

void SkSVGDevice::onLayerRestore() {
    if (currentLayerFilter) {
        if (currentLayerFilter->size() > 0 && currentLayerFilter->isLayerFilterAdded()) {
            currentLayerFilter->getElement()->fWriter->endElement();
            currentLayerFilter->pop();
        }
    }
}
