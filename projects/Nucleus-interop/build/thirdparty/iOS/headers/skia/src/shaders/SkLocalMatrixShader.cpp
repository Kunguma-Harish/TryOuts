...
bool SkLocalMatrixShader::onReplaceShaderProps(sk_sp<SkShader> shader) {
    SkLocalMatrixShader* localMatrixShader = static_cast<SkLocalMatrixShader*>(shader.get());
    fLocalMatrix = localMatrixShader->fLocalMatrix;
    fWrappedShader = localMatrixShader->fWrappedShader;
    if (!localMatrixShader->fWrappedShader) {
        return false;
    }
    return as_SB(localMatrixShader->fWrappedShader.get())->onReplaceShaderProps(localMatrixShader->fWrappedShader);
}
