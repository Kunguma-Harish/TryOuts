...
@implementation ZMacAuthKeyPairUtil
...
- (NSData *)readKeyBits:(NSData *)tag keyType:(CFTypeRef)keyType {
    ...
    sanityCheck = SecItemCopyMatching((__bridge CFDictionaryRef)queryPublicKey, (CFTypeRef *)&priKey);
    CFErrorRef error1 = NULL;
    NSData* keyData = (NSData*)CFBridgingRelease(  // ARC takes ownership
                           SecKeyCopyExternalRepresentation(priKey, &error1)
                       );
    if (!keyData) {
        //NSError *err1 = CFBridgingRelease(error1);  // ARC takes ownership
        // Handle the error. . .
    }
    return keyData;
...
- (NSString *)rsaDecryptWithData:(NSData*)data usingPublicKey:(BOOL)yes{
    ...
    CFErrorRef error1 = NULL;
    NSData* keyData = (NSData*)CFBridgingRelease(  // ARC takes ownership
                                                 SecKeyCreateDecryptedData(key, kSecKeyAlgorithmRSAEncryptionPKCS1, ((__bridge CFDataRef)data), &error1)
                       );
    if (!keyData) {
        //NSError *err1 = CFBridgingRelease(error1);  // ARC takes ownership
        // Handle the error. . .
    ...
    */
    NSString *dataString = [[NSString alloc] initWithData:keyData
    encoding:NSUTF8StringEncoding];
    if (dataString == nil) {
        dataString = [keyData base64EncodedStringWithOptions:NSDataBase64EncodingEndLineWithCarriageReturn];
    }
    return dataString;
