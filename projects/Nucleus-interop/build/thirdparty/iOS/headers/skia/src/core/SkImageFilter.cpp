...
void SkImageFilter::setBounds(SkRect bounds) { this->filterBound = bounds; }

const SkRect SkImageFilter::getBounds() { return this->filterBound; }


std::shared_ptr<SkImageFilter::shadowAttributes> SkImageFilter::extractOuterShadowAttributes(
        SkImageFilter* shadowFilter) {
    std::shared_ptr<shadowAttributes> shadow = std::make_shared<shadowAttributes>();
    
    SkMatrix shadowTransformMatrix = SkImageFilter_Base::getTransformFromFilter(const_cast<SkImageFilter*>(shadowFilter->getInput(0)));
    SkRect shadowBoundary = (const_cast<SkImageFilter*>(shadowFilter->getInput(0)))->getBounds();    
    shadow->color = SkImageFilter_Base::getColorFromColorFilter(const_cast<SkImageFilter*>(shadowFilter->getInput(0)->getInput(0)));
    shadow->sigma =  SkImageFilter_Base::getBlurFromFilter(const_cast<SkImageFilter*>(shadowFilter->getInput(0)->getInput(0)->getInput(0)));
    shadow->width = shadowBoundary.width();
    shadow->height = shadowBoundary.height();
    shadow->dx = shadowTransformMatrix.getTranslateX();
    shadow->dy = shadowTransformMatrix.getTranslateY();
    shadow->type = "outer";
    return shadow;
}

std::shared_ptr<SkImageFilter::shadowAttributes> SkImageFilter::extractInnerShadowAttributes(
        SkImageFilter* shadowFilter) {
    std::shared_ptr<shadowAttributes> shadow = std::make_shared<shadowAttributes>();
    if (std::string(shadowFilter->getTypeName()) == "SkBlendImageFilter" && std::string(shadowFilter->getInput(0)->getTypeName()) == "SkBlendImageFilter" && std::string(shadowFilter->getInput(0)->getInput(0)->getTypeName()) == "SkBlendImageFilter") {
        shadowFilter = const_cast<SkImageFilter*>(shadowFilter->getInput(0));
    }
    shadow = extractOuterShadowAttributes((const_cast<SkImageFilter*>(shadowFilter->getInput(0))));
    shadow->color = SkImageFilter_Base::getColorFromColorMatrixFilter(const_cast<SkImageFilter*>(shadowFilter->getInput(0)->getInput(0)->getInput(0)->getInput(0)->getInput(0)));
    SkRect shadowBoundary =  const_cast<SkImageFilter*>(shadowFilter->getInput(0))->getBounds();
    shadow->width = shadowBoundary.width();
    shadow->height = shadowBoundary.height();
    shadow->type = "inner";
...
std::shared_ptr<SkImageFilter::shadowAttributes> SkImageFilter::extractBlurAttributes(
        SkImageFilter* imagefilter) {
    std::shared_ptr<shadowAttributes> shadow = std::make_shared<shadowAttributes>();
    shadow->sigma = SkImageFilter_Base::getBlurFromFilter(imagefilter);
    return shadow;
}

/*
Checks for filter pattern as 
 ** SkMatrixTransformImageFilter-SkBlurImageFilter-SkColorFilterImageFilter
 ** SkBlendImageFilter-SkMatrixTransformImageFilter-SkBlurImageFilter-SkColorFilterImageFilter
*/

bool SkImageFilter::isDropShadowFilter(SkImageFilter* filter) {
    if (!filter) {
        return false;
    }

    const SkImageFilter* currentFilter = filter;

    if (std::string(currentFilter->getTypeName()) == "SkBlendImageFilter") {
        if (std::string(currentFilter->getInput(0)->getTypeName()) == "SkMatrixTransformImageFilter" && currentFilter->countInputs() == 1) {
            currentFilter = currentFilter->getInput(0);
        } else {
            return false;
        }
    }

    // Check for optional crop
    // if (currentFilter->getTypeName() == "SkCropImageFilter") {
    //     currentFilter = currentFilter->getInput(0);
    // }

    // Check for optional blend or merge
    // if (!currentFilter->getInput(1) && (currentFilter->getTypeName() == "SkBlendImageFilter" || currentFilter->getTypeName() == "SkMergeImageFilter")) {
    //     currentFilter = currentFilter->getInput(0);
    // }

    if (!currentFilter || !(std::string(currentFilter->getTypeName()) == "SkMatrixTransformImageFilter")) {
        return false;
    }
    currentFilter = currentFilter->getInput(0);

    if (!currentFilter || !(std::string(currentFilter->getTypeName()) == "SkColorFilterImageFilter")) {
        return false;
    }
    currentFilter = currentFilter->getInput(0);

    if (!currentFilter || !(std::string(currentFilter->getTypeName()) == "SkBlurImageFilter")) {
        return false;
    }
...
/*
Checks for filter pattern as 
 ** SkBlendImageFilter-SkMatrixTransformImageFilter,SkColorFilterImageFilter-SkBlurImageFilter-SkColorFilterImageFilter
 ** SkBlendImageFilter-SkBlendImageFilter-SkMatrixTransformImageFilter,SkColorFilterImageFilter-SkBlurImageFilter-SkColorFilterImageFilter
*/
bool SkImageFilter::isInnerShadowFilter(SkImageFilter* filter) {
    if (!filter) {
        return false;
    }
    ...
    const SkImageFilter* currentFilter = filter;
    if (std::string(currentFilter->getTypeName()) == "SkBlendImageFilter") {
        if (std::string(currentFilter->getInput(0)->getTypeName()) == "SkBlendImageFilter"){
            currentFilter = currentFilter->getInput(0);
        }
    }
    ...
    if (!(std::string(currentFilter->getInput(1)->getTypeName()) == "SkColorFilterImageFilter")) {
        return false;
    }
    ...
    // if (!currentFilter->getInput(1) || !(currentFilter->getInput(1)->getTypeName() == "SkColorFilterImageFilter")) {
    //     return false;
    // }
    
    // if (currentFilter->getTypeName() == "SkBlendImageFilter") {
    //     currentFilter = currentFilter->getInput(0);
    // }
    ...
    // // Check for optional blend or merge
    // if (currentFilter->getTypeName() == "SkBlendImageFilter" || currentFilter->getTypeName() == "SkMergeImageFilter") {
    //     currentFilter = currentFilter->getInput(0);
    // }
