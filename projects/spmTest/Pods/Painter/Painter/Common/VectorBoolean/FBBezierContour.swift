...
class FBBezierContour {
	...
	private var _cgPathCache: CGPath?
	...
	func addCurve(_ curve: FBBezierCurve?) {
		...
		if let curve = curve {
			curve.contour = self
			curve.index = self._edges.count
			self._edges.append(curve)
			self._bounds = CGRect.null // force the bounds to be recalculated
			self._boundingRect = CGRect.null
			self._cgPathCache = nil
		}
	}
	...
	var boundingRect: CGRect {
		...
		if !self._boundingRect.equalTo(CGRect.null) {
		...
		self._boundingRect = totalBounds
	...
	var firstPoint: CGPoint {
		...
		return self._edges[0].endPoint1
	}

	// 189
	// - (BOOL) containsPoint:(NSPoint)testPoint
	func containsPoint(_ testPoint: CGPoint) -> Bool {
		if !self.boundingRect.contains(testPoint) || !self.bounds.contains(testPoint) {
			return false
		}
		...
		let externalXPt = testPoint.x > self.bounds.minX ? self.bounds.minX - 10 : self.bounds.maxX + 10
		...
		let intersectCount = self.numberOfIntersectionsWithRay(testCurve)
	...
	func numberOfIntersectionsWithRay(_ testEdge: FBBezierCurve) -> Int {
		var count = 0
		self.intersectionsWithRay(testEdge, withBlock: {
			(_: FBBezierIntersection) in
			count += 1
		})
	...
	func markCrossingsAsEntryOrExitWithContour(_ otherContour: FBBezierContour, markInside: Bool) {
		...
		let (startEdge, startParameter, startPoint) = self.startingEdge()
		...
		isEntry = self.markCrossingsOnEdge(startEdge, startParameter: startParameter, stopParameter: FBStopParameterNoLimit, otherContours: otherContours, startIsEntry: isEntry)

		var edge = startEdge.next
		while edge !== startEdge {
			isEntry = self.markCrossingsOnEdge(edge, startParameter: FBStartParameterNoLimit, stopParameter: FBStopParameterNoLimit, otherContours: otherContours, startIsEntry: isEntry)
			edge = edge.next
		}

		self.markCrossingsOnEdge(startEdge, startParameter: FBStartParameterNoLimit, stopParameter: startParameter, otherContours: otherContours, startIsEntry: isEntry)
	}
	...
	func markCrossingsOnEdge(_ edge: FBBezierCurve, startParameter: Double, stopParameter: Double, otherContours: [FBBezierContour], startIsEntry: Bool) -> Bool {
		...
		edge.crossingsWithBlock {
			...
			isEntry.toggle() // toggle to indicate exit or entry
			return (false, false)
		}
	...
	private func contourAndSelfIntersectingContoursContainPoint(_ point: CGPoint) -> Bool {
		var containerCount = 0
		if self.containsPoint(point) {
			containerCount += 1
		}
		for contour in self.selfIntersectingContours {
	...
	var cgPath: CGPath {
		if self._cgPathCache == nil {
			...
			self._cgPathCache = path
		}

		return self._cgPathCache!
	}
	...
	func close() {
		...
		let first = self._edges[0]
		if let last = _edges.last {
			if !FBArePointsClose(first.endPoint1, point2: last.endPoint2) {
				self.addCurve(FBBezierCurve(startPoint: last.endPoint2, endPoint: first.endPoint1))
			}
		}
	}
	...
	var intersectingContours: [FBBezierContour] {
		...
		for edge in self._edges {
			edge.intersectingEdgesWithBlock {
				(intersectingEdge: FBBezierCurve) in
				if let ieContour = intersectingEdge.contour {
					if iContours.filter({ el in el === ieContour }).isEmpty {
	...
	var selfIntersectingContours: [FBBezierContour] {
		...
		self.addSelfIntersectingContoursToArray(&siContours, originalContour: self)
		return siContours
	}

	// 499
	// - (void) addSelfIntersectingContoursToArray:(NSMutableArray *)contours originalContour:(FBBezierContour *)originalContour
	private func addSelfIntersectingContoursToArray(_ contours: inout [FBBezierContour], originalContour: FBBezierContour) {
		for edge in self._edges {
			edge.selfIntersectingEdgesWithBlock {
				(intersectingEdge: FBBezierCurve) in
				if let ieContour = intersectingEdge.contour {
					if ieContour !== originalContour, contours.filter({ el in el === ieContour }).isEmpty {
	...
	private func forEachEdgeOverlapDo(_ block: (_ overlap: FBEdgeOverlap) -> Void) {
		if self._overlaps.isEmpty {
			return
		}
