...
int SkCanvas::saveLayer(const SaveLayerRec& rec) {
    ...
    if (rec.fPaint && rec.fPaint->nothingToDraw()) {
        ...
        if (rec.fPaint) {
            if ((rec.fPaint->getAlphaf() != 1 && rec.fPaint->getAlphaf() != 0) || rec.fPaint->getImageFilter()) {
                SkDeque::Iter iter(fMCStack, SkDeque::Iter::kBack_IterStart);
                for (;;) {
                    MCRec* mcrec = (MCRec*)iter.next();
                    if (!mcrec) {
                        break;
                    }
                    mcrec->fDevice->onSaveLayer(const_cast<SkPaint*>(rec.fPaint));    
                }   
            }
        } 
...
void SkCanvas::internalSaveLayer(const SaveLayerRec& rec,
    ...
    if (!newDevice) {
        ...
        // doesn't exist.
        //NOTE: Commenting this so as to not wipre out prior device : need to make svgdevice::createDevice work
