...
@interface ZMacAuthKeyChainWrapper () {
    NSMutableDictionary *itemsToUpdate;
}

@end

@implementation ZMacAuthKeyChainWrapper

+ (NSString *)defaultService
{
    if (!_defaultService) {
        _defaultService = [ZMacAuthZIAMUtil sharedUtil]->AppName;
    }
...
+ (ZMacAuthKeyChainWrapper *)keyChainStore
{
...
- (instancetype)init
{
    return [self initWithService:[self.class defaultService] accessGroup:[ZMacAuthZIAMUtil sharedUtil].ExtensionAppGroup];
}

- (instancetype)initWithService:(NSString *)service
{
    return [self initWithService:service accessGroup:[ZMacAuthZIAMUtil sharedUtil].ExtensionAppGroup];
}
...
{
    return [self stringForKey:key service:[self defaultService] accessGroup:[ZMacAuthZIAMUtil sharedUtil].ExtensionAppGroup];
}

+ (NSString *)stringForKey:(NSString *)key service:(NSString *)service
{
    return [self stringForKey:key service:service accessGroup:[ZMacAuthZIAMUtil sharedUtil].ExtensionAppGroup];
}
...
{
    BOOL __block flag;    
        flag =  [self setString:value forKey:key service:[self defaultService] accessGroup:[ZMacAuthZIAMUtil sharedUtil].ExtensionAppGroup];
    return flag;
...
+ (BOOL)setString:(NSString *)value forKey:(NSString *)key service:(NSString *)service
{
    ...
    return [self setString:value forKey:key service:service accessGroup:[ZMacAuthZIAMUtil sharedUtil].ExtensionAppGroup];
}
...
{
    return [self dataForKey:key service:[self defaultService] accessGroup:[ZMacAuthZIAMUtil sharedUtil].ExtensionAppGroup];
}

+ (NSData *)dataForKey:(NSString *)key service:(NSString *)service
{
    return [self dataForKey:key service:service accessGroup:[ZMacAuthZIAMUtil sharedUtil].ExtensionAppGroup];
}
...
+ (NSData *)dataForKey:(NSString *)key service:(NSString *)service accessGroup:(NSString *)accessGroup
{
    ...
    if([ZMacAuthZIAMUtil sharedUtil].SharedTargetPaths){
        [self createAccessForQuery:query service:[ZMacAuthZIAMUtil sharedUtil].KeychainService];
    }
...
{
    return [self setData:data forKey:key service:[self defaultService] accessGroup:[ZMacAuthZIAMUtil sharedUtil].ExtensionAppGroup];
}

+ (BOOL)setData:(NSData *)data forKey:(NSString *)key service:(NSString *)service
{
    return [self setData:data forKey:key service:service accessGroup:[ZMacAuthZIAMUtil sharedUtil].ExtensionAppGroup];
}

+ (BOOL)createAccessForQuery:(NSMutableDictionary *)query service:(NSString *)service;
{
    ...
    for (NSString *app in [ZMacAuthZIAMUtil sharedUtil].SharedTargetPaths) {
    ...
    //Create an access object:
     _keychainStatus = SecAccessCreate((__bridge CFStringRef)service,
...
+ (BOOL)setData:(NSData *)data forKey:(NSString *)key service:(NSString *)service accessGroup:(NSString *)accessGroup
{
    ...
    [query setObject:key forKey:(__bridge id)kSecAttrGeneric];
    [query setObject:key forKey:(__bridge id)kSecAttrAccount];
    [query setObject:kCFBooleanTrue forKey:(__bridge id)kSecAttrIsInvisible];
    if (accessGroup) {
        [query setObject:accessGroup forKey:(__bridge id)kSecAttrAccessGroup];
    }
    if([ZMacAuthZIAMUtil sharedUtil].SharedTargetPaths){
    ...
    if (status == errSecSuccess) {
        ...
        [attributes setObject:kCFBooleanTrue forKey:(__bridge id)kSecAttrIsInvisible];
        ...
        if([ZMacAuthZIAMUtil sharedUtil].SharedTargetPaths){
            [self createAccessForQuery:attributes service:[ZMacAuthZIAMUtil sharedUtil].KeychainService];
        }
...
{
    return [ZMacAuthKeyChainWrapper removeItemForKey:key service:[self defaultService] accessGroup:[ZMacAuthZIAMUtil sharedUtil].ExtensionAppGroup];
}

+ (BOOL)removeItemForKey:(NSString *)key service:(NSString *)service
{
    return [ZMacAuthKeyChainWrapper removeItemForKey:key service:service accessGroup:[ZMacAuthZIAMUtil sharedUtil].ExtensionAppGroup];
}
...
+ (BOOL)removeItemForKey:(NSString *)key service:(NSString *)service accessGroup:(NSString *)accessGroup
{
    ...
    if([ZMacAuthZIAMUtil sharedUtil].SharedTargetPaths){
        [self createAccessForQuery:itemToDelete service:[ZMacAuthZIAMUtil sharedUtil].KeychainService];
    }
...
+ (BOOL)removeAllItems
{
    return [self removeAllItemsForService:[self defaultService] accessGroup:[ZMacAuthZIAMUtil sharedUtil].ExtensionAppGroup];
}

+ (BOOL)removeAllItemsForService:(NSString *)service
{
    return [self removeAllItemsForService:service accessGroup:[ZMacAuthZIAMUtil sharedUtil].ExtensionAppGroup];
}

+ (BOOL)removeAllItemsForService:(NSString *)service accessGroup:(NSString *)accessGroup
{
    NSArray *items = [ZMacAuthKeyChainWrapper itemsForService:service accessGroup:accessGroup];
...
{
    NSArray *items = [ZMacAuthKeyChainWrapper itemsForService:self.service accessGroup:self.accessGroup];
