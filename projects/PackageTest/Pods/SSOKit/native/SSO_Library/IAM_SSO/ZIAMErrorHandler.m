...
-(void)handleUpdatePhotoError:(SSOInternalError)type info:(NSError*)error failureBlock:(requestFailureBlock)failed {
    if (type == SSO_ERR_JSON_NIL) {
        ...
        DLog(@"UpdatePhoto Nothing was received");
        NSMutableDictionary* userInfo = [[NSMutableDictionary alloc] init];
        [userInfo setValue:@"UpdatePhoto nothing was received " forKey:NSLocalizedDescriptionKey];
        NSError *error = [NSError errorWithDomain:kSSOKitErrorDomain code:k_SSOUpdatePhotoNothingReceived userInfo:userInfo];
        failed(error);
