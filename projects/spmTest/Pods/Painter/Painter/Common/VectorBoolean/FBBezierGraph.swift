...
public class FBBezierGraph {
	...
	public init(path: CGPath) {
		self._contours = []
		self._bounds = CGRect.null
		_ = self.initWithBezierPath(path)
	}

	class func bezierGraphWithBezierPath(_ path: CGPath!) -> AnyObject {
		return FBBezierGraph().initWithBezierPath(path)
	}

	// - (id) initWithBezierPath:(NSBezierPath *)path
	func initWithBezierPath(_ path: CGPath!) -> FBBezierGraph {
		...
		for (_, elem) in bezier.elements.enumerated() {
			...
			case let .cubicCurve(toPt, v1, v2):
	...
	func unionWithBezierGraph(_ graph: FBBezierGraph) -> FBBezierGraph! {
		...
		var result = self.bezierGraphFromIntersections
	...
	private func unionNonintersectingPartsIntoGraph(_ result: inout FBBezierGraph, withGraph graph: FBBezierGraph) {
		// Finally, process the contours that don't cross anything else. They're either
		//  completely contained in another contour, or disjoint.
	...
	private func unionEquivalentNonintersectingContours(_ ourNonintersectingContours: inout [FBBezierContour], withContours theirNonintersectingContours: inout [FBBezierContour], results: inout [FBBezierContour]) {
		var ourIndex = 0
	...
	private func intersectNonintersectingPartsIntoGraph(_ result: inout FBBezierGraph, withGraph graph: FBBezierGraph) {
		// Finally, process the contours that don't cross anything else. They're either
		//  completely contained in another contour, or disjoint.
		var ourNonintersectingContours = self.nonintersectingContours
		var theirNonintersectinContours = graph.nonintersectingContours
		var finalNonintersectingContours = self.intersectEquivalentNonintersectingContours(&ourNonintersectingContours, withContours: &theirNonintersectinContours)
	...
	private func intersectEquivalentNonintersectingContours(_ ourNonintersectingContours: inout [FBBezierContour], withContours theirNonintersectingContours: inout [FBBezierContour]) -> [FBBezierContour] {
	...
	func differenceWithBezierGraph(_ graph: FBBezierGraph) -> FBBezierGraph {
		...
		// Walk the crossings and actually compute the final result for the intersecting parts
		let result = self.bezierGraphFromIntersections

		// Finally, process the contours that don't cross anything else. They're either
		//  completely contained in another contour, or disjoint.
		var ourNonintersectingContours = self.nonintersectingContours
		var theirNonintersectinContours = graph.nonintersectingContours
		var finalNonintersectingContours = self.differenceEquivalentNonintersectingContours(&ourNonintersectingContours, withContours: &theirNonintersectinContours)
	...
	private func differenceEquivalentNonintersectingContours(_ ourNonintersectingContours: inout [FBBezierContour], withContours theirNonintersectingContours: inout [FBBezierContour]) -> [FBBezierContour] {
		var results: [FBBezierContour] = []
	...
	func xorWithBezierGraph(_ graph: FBBezierGraph) -> FBBezierGraph {
		...
		// Walk the crossings and actually compute the final result for the intersecting parts
		var allParts = self.bezierGraphFromIntersections
		...
		var intersectingParts = self.bezierGraphFromIntersections
		...
		for contour in self._contours {
			...
			for edge in contour.edges {
				...
				if edge.isStraightLine {
					...
					path.addCurve(to: edge.endPoint2, control1: edge.controlPoint1, control2: edge.controlPoint2)
				}
			}
			if !path.isEmpty {
				path.closeSubpath() // GPC: close each contour
			}
		}
	...
	internal func insertCrossingsWithBezierGraph(_ other: FBBezierGraph) {
		...
		for ourContour in self.contours {
	...
	internal func insertSelfCrossings() {
		...
		while !remainingContours.isEmpty {
	...
	var bounds: CGRect {
		...
		if !self._bounds.equalTo(CGRect.null) {
			return self._bounds
		}

		if self._contours.isEmpty {
			return CGRect.zero
		}

		for contour in self._contours {
			self._bounds = self._bounds.union(contour.bounds)
		}
	...
	private func contourInsides(_ testContour: FBBezierContour) -> FBContourInside {
	...
	public func closestLocationToPoint(_ point: CGPoint) -> FBCurveLocation? {
		var closestLocation: FBCurveLocation?
	...
	//  func debugPathForContainmentOfContour(_ testContour: FBBezierContour) -> CGPath {
	...
	//  func debugPathForContainmentOfContour(_ testContour: FBBezierContour, transform: CGAffineTransform) -> CGPath {
	//    let path = CGMutablePath()
//
	//    var intersectCount = 0
	//    for contour in self.contours {
...
//
	...
	private func containsContour(_ testContour: FBBezierContour) -> Bool {
		...
		for fraction in 2...count * 2 {
			...
			while y < yFinir {
				...
				let eliminated = self.eliminateContainers(&containers, thatDontContainContour: testContour, usingRay: ray)
				if eliminated {
					didEliminate = true
				}
				y += verticalSpacing
	...
	private func findBoundsOfContour(_ testContour: FBBezierContour, onRay ray: FBBezierCurve, minimum testMinimum: inout CGPoint, maximum testMaximum: inout CGPoint) -> Bool {
	...
	private func findCrossingsOnContainers(_ containers: [FBBezierContour], onRay ray: FBBezierCurve, beforeMinimum testMinimum: CGPoint, afterMaximum testMaximum: CGPoint, crossingsBefore crossingsBeforeMinimum: inout [FBEdgeCrossing], crossingsAfter crossingsAfterMaximum: inout [FBEdgeCrossing]) -> Bool {
		...
		for ambiguousCrossing in ambiguousCrossings {
			...
			if let ambigEdge = ambiguousCrossing.edge, let edgeContour = ambigEdge.contour {
				let numberOfTimesContourAppearsBefore = self.numberOfTimesContour(edgeContour, appearsInCrossings: crossingsBeforeMinimum)
				let numberOfTimesContourAppearsAfter = self.numberOfTimesContour(edgeContour, appearsInCrossings: crossingsAfterMaximum)
	...
	private func numberOfTimesContour(_ contour: FBBezierContour, appearsInCrossings crossings: [FBEdgeCrossing]) -> Int {
	...
	private func eliminateContainers(_ containers: inout [FBBezierContour], thatDontContainContour testContour: FBBezierContour, usingRay ray: FBBezierCurve) -> Bool {
		...
		let foundBounds = self.findBoundsOfContour(testContour, onRay: ray, minimum: &testMinimum, maximum: &testMaximum)
		...
		let foundCrossings = self.findCrossingsOnContainers(containers, onRay: ray, beforeMinimum: testMinimum, afterMaximum: testMaximum, crossingsBefore: &crossingsBeforeMinimum, crossingsAfter: &crossingsAfterMaximum)
		...
		containers = self.contoursFromCrossings(crossingsBeforeMinimum)
	...
	private func contoursFromCrossings(_ crossings: [FBEdgeCrossing]) -> [FBBezierContour] {
		...
		for crossing in crossings {
			if let crossingEdge = crossing.edge {
				if let contour = crossingEdge.contour {
					...
					if contours.filter({ el in el === contour }).isEmpty {
	...
	private func removeContourCrossings(_ crossings1: inout [FBEdgeCrossing], thatDontAppearIn crossings2: [FBEdgeCrossing]) {
	...
	private func removeContoursThatDontContain(_ crossings: inout [FBEdgeCrossing]) {
	...
	private func removeCrossings(_ crossings: inout [FBEdgeCrossing], forContours containersToRemove: [FBBezierContour]) {
	...
	private var bezierGraphFromIntersections: FBBezierGraph {
		...
		var optCrossing: FBEdgeCrossing? = self.firstUnprocessedCrossing
		while var crossing = optCrossing {
				...
				if crossing.isEntry {
					...
					if let prevNonself = crossing.previousNonself {
						...
						if let crossingEdge = crossing.edge { // should ALWAYS be the case
                            ...
                            Debugger.debug("This is bad, really bad")
						}
					}
				}
			...
			optCrossing = self.firstUnprocessedCrossing
		}
	...
	private func addContour(_ contour: FBBezierContour) {
	...
	private var nonintersectingContours: [FBBezierContour] {
		...
		var contours: [FBBezierContour] = []
		for contour in self.contours {
			if contour.intersectingContours.isEmpty {
				contours.append(contour)
			}
		}
		return contours
	...
	func debuggingInsertCrossingsForUnionWithBezierGraph(_ otherGraph: inout FBBezierGraph) {
		self.debuggingInsertCrossingsWithBezierGraph(&otherGraph, markInside: false, markOtherInside: false)
	}
	...
	func debuggingInsertCrossingsForIntersectWithBezierGraph(_ otherGraph: inout FBBezierGraph) {
		self.debuggingInsertCrossingsWithBezierGraph(&otherGraph, markInside: true, markOtherInside: true)
	}
	...
	func debuggingInsertCrossingsForDifferenceWithBezierGraph(_ otherGraph: inout FBBezierGraph) {
		self.debuggingInsertCrossingsWithBezierGraph(&otherGraph, markInside: false, markOtherInside: true)
	}
	...
	private func debuggingInsertCrossingsWithBezierGraph(_ otherGraph: inout FBBezierGraph, markInside: Bool, markOtherInside: Bool) {
		// Clean up crossings so the graphs can be reused, e.g. XOR will reuse graphs.
		self.removeCrossings()
	...
	private func debuggingInsertIntersectionsWithBezierGraph(_ otherGraph: inout FBBezierGraph) {
		// Clean up crossings so the graphs can be reused, e.g. XOR will reuse graphs.
		self.removeCrossings()
		otherGraph.removeCrossings()
		self.removeOverlaps()
