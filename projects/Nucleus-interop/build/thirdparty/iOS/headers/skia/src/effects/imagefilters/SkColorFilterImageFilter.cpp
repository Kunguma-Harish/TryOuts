...
public:
    ...
    SkColorFilter* getColorFilter() { return fColorFilter.get(); }
...
SkColor SkImageFilter_Base::getColorFromColorFilter(SkImageFilter* filter)  {
    SkColorFilterImageFilter* colorFilterImageFilter = ((SkColorFilterImageFilter*)as_IFB(filter));
    return ((SkBlendModeColorFilter*)as_CFB(colorFilterImageFilter->getColorFilter()))->color().toSkColor() ;//->color().toSkColor());
}


SkColor SkImageFilter_Base::getColorFromColorMatrixFilter(SkImageFilter* filter)  {
    SkColorFilterImageFilter* colorFilterImageFilter = ((SkColorFilterImageFilter*)as_IFB(filter));
     
    const float* matrixPtr = SkColorFilterBase::getColorMatrix(colorFilterImageFilter->getColorFilter());
    float copyMatrix[20];
    std::copy(matrixPtr, matrixPtr + 20, copyMatrix);


    constexpr int kR_Trans = 4;
    constexpr int kG_Trans = 9;
    constexpr int kB_Trans = 14;
    constexpr int kA_Trans = 19;

    float r = matrixPtr[kR_Trans] * 255.0f;
    float g = matrixPtr[kG_Trans] * 255.0f;
    float b = matrixPtr[kB_Trans] * 255.0f;
    float a = matrixPtr[kA_Trans] * 255.0f;

    SkColor color = SkColorSetARGB(static_cast<U8CPU>(std::clamp(a, 0.0f, 255.0f)),
                          static_cast<U8CPU>(std::clamp(r, 0.0f, 255.0f)),
                          static_cast<U8CPU>(std::clamp(g, 0.0f, 255.0f)),
                          static_cast<U8CPU>(std::clamp(b, 0.0f, 255.0f)));
    return color;
}
