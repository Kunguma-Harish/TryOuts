...
class FBBezierIntersection {
	...
	var isTangent: Bool {
		...
		let curve1LeftTangent = FBNormalizePoint(FBSubtractPoint(_curve1LeftBezier!.controlPoint2, point2: self._curve1LeftBezier!.endPoint2))
		let curve1RightTangent = FBNormalizePoint(FBSubtractPoint(_curve1RightBezier!.controlPoint1, point2: self._curve1RightBezier!.endPoint1))
		let curve2LeftTangent = FBNormalizePoint(FBSubtractPoint(_curve2LeftBezier!.controlPoint2, point2: self._curve2LeftBezier!.endPoint2))
		let curve2RightTangent = FBNormalizePoint(FBSubtractPoint(_curve2RightBezier!.controlPoint1, point2: self._curve2RightBezier!.endPoint1))
	...
	var isAtStartOfCurve1: Bool {
		return FBAreValuesCloseWithOptions(self._parameter1, value2: 0.0, threshold: FBParameterCloseThreshold) || self._curve1.isPoint
	}

	// - (BOOL) isAtStopOfCurve1
	var isAtStopOfCurve1: Bool {
		return FBAreValuesCloseWithOptions(self._parameter1, value2: 1.0, threshold: FBParameterCloseThreshold) || self._curve1.isPoint
	}
	...
	var isAtStartOfCurve2: Bool {
		return FBAreValuesCloseWithOptions(self._parameter2, value2: 0.0, threshold: FBParameterCloseThreshold) || self._curve2.isPoint
	}

	// - (BOOL) isAtStopOfCurve2
	var isAtStopOfCurve2: Bool {
		return FBAreValuesCloseWithOptions(self._parameter2, value2: 1.0, threshold: FBParameterCloseThreshold) || self._curve2.isPoint
	}
	...
	private func computeCurve1() {
		if self.needToComputeCurve1 {
			let pap = self._curve1.pointAtParameter(self._parameter1)
			self._location = pap.point
			self._curve1LeftBezier = pap.leftBezierCurve
			self._curve1RightBezier = pap.rightBezierCurve
	...
	private func computeCurve2() {
		if self.needToComputeCurve2 {
			let pap = self._curve2.pointAtParameter(self._parameter2)
			// not using the point from curve2
			self._curve2LeftBezier = pap.leftBezierCurve
			self._curve2RightBezier = pap.rightBezierCurve
