...
bool SkImageShader::onReplaceShaderProps(sk_sp<SkShader> shader) {
    SkImageShader* imageShader = static_cast<SkImageShader*>(shader.get());
    if (!imageShader->fImage) {
        return false;
    }
    fImage = imageShader->fImage;
    fTileModeX = imageShader->fTileModeX;
    fTileModeY = imageShader->fTileModeY;
    return true;
}