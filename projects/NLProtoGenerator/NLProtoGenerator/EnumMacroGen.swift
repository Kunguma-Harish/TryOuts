...
//  NLProtoGenerator
//
//  Created by kunguma-14252 on 20/05/25.
//
...
class EnumMacroGen {
    ...
    func parentType(from fullTypeName: String) -> String? {
    ...
    func generateHpp() -> (name: String, content: String) {
        ...
        if let parentClass = parentType(from: objcType) {
            lines.append("#include \"\(parentClass).hpp\"")
        }
        lines.append("")
...
//            lines.append("")
        ...
        lines.append("@interface \(wrapperClassName) : NSObject")
        lines.append("#ifdef __cplusplus \n")
        lines.append("-(instancetype)init:(const google::protobuf::RepeatedField<int>*)pointer; \n")
        lines.append("#endif \n")
        lines.append("-(NSUInteger)count;")
        lines.append("-(\(objcType))get:(NSUInteger)index; \n")
        lines.append("@end \n")
        lines.append("")
        lines.append("#endif")
    ...
    func generateMm() -> (name: String, content: String) {
        ...
        lines.append("@implementation \(wrapperClassName) { \n")
        lines.append("const google::protobuf::RepeatedField<int> *rptPointer; \n")
        lines.append("} \n")
        lines.append("- (instancetype)init:(const google::protobuf::RepeatedField<int>*)pointer {")
        lines.append("self = [super init];")
        lines.append("if (self) {")
        lines.append("self->rptPointer = pointer;")
        lines.append("} \n")
        lines.append("return self;")
        lines.append("} \n")
        lines.append("- (NSUInteger)count {")
        lines.append("return self->rptPointer->size();")
        lines.append("} \n")
        lines.append("- (\(objcType))get:(NSUInteger)index {")
        lines.append("auto rvalue = self->rptPointer->Get(index);")
        lines.append("return (\(objcType))rvalue;")
        lines.append("}")
        lines.append("@end \n")
        let content = lines.joined(separator: "\n")
